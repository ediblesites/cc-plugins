...
1. **Add the marketplace**: From within Claude Code, run the `plugin marketplace add` command:
   ```shell
   /plugin marketplace add anthropics/claude-code
   ```

2. **Browse available plugins**: Run `/plugin` to open the plugin manager. This opens a tabbed interface with four tabs you can cycle through using **Tab** (or **Shift+Tab** to go backward):
...
3. **Install a plugin**: Select a plugin to view its details, then choose an installation scope:
...
4. **Use your new plugin**: After installing, the plugin's commands are immediately available. Plugin commands are namespaced by the plugin name, so **commit-commands** provides commands like `/commit-commands:commit`.
...
> URL-based marketplaces have some limitations compared to Git-based marketplaces.
...
The `--scope` option lets you target a specific scope:
...
To disable all automatic updates entirely:
